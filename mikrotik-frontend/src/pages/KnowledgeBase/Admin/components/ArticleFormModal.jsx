...
import { 
  ...
  Terminal, Code as CodeIcon, Eye, Globe, ShieldAlert, User, File, Download, Paperclip, Trash2
} from 'lucide-react';
...
const ArticleFormModal = ({ isOpen, onClose, articleId, onSaveSuccess }) => {
  ...
  const handleAttachUpload = async (e) => {
    ...
    if (file.size > 20 * 1024 * 1024) {
      return Swal.fire({
        title: t('common.error'),
        text: "ไฟล์ต้องมีขนาดไม่เกิน 20MB",
        icon: 'error',
        confirmButtonText: 'ตกลง'
      });
    }
    ...
    try {
      ...
      const res = await uploadAttachment(articleId, file);
  ...
  const handleDeleteAttach = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await articleService.deleteAttachment(id);
  ...
  const handleToggleAttachVisibility = async (id) => {
    try {
      const res = await articleService.toggleAttachmentVisibility(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!articleId && res?.id) {
        // If it was a new article, we might need its ID if we want to stay open, 
        // but here we follow original logic of closing.
      }
      onSaveSuccess();
      onClose();
    } catch (error) {
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-8 bg-slate-50/30 custom-scrollbar">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-slate-900">
                    <div className="lg:col-span-2 space-y-6">
                      ...
                      {/* --- Section: จัดการไฟล์แนบ (เฉพาะเมื่อแก้ไขบทความ) --- */}
                              ...
                              {[
                                ...
                                >
                                  ...
                                  <div className="flex flex-col items-start text-left">
                                    <span className={formData.visibility === v.id ? 'text-slate-800' : ''}>{t(`articles.vis_${v.id.toLowerCase()}`, v.id)}</span>
                                  </div>
                                  {formData.visibility === v.id && (
