...
import { Shield, Network, Globe, Save, Plus, Trash2, AlertTriangle, Loader2 } from 'lucide-react';
...
const GlobalSettings = () => {
  ...
  if (isLoading) return <div className="flex justify-center items-center h-64"><Loader2 className="animate-spin text-blue-500" size={48} /></div>;
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm min-h-[400px] flex flex-col">
            ...
            <div className="space-y-4">
              {routerAdmins.map((admin, idx) => (
                <div key={idx} className="flex flex-wrap md:flex-nowrap items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
                  ...
                  <input type="text" value={admin.password} onChange={(e) => updateAdmin(idx, 'password', e.target.value)} placeholder="Password" className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" />
              ...
              <div className="flex flex-wrap md:flex-nowrap gap-3 pt-4 mt-4 border-t border-slate-100">
                ...
                <input type="password" placeholder="New Password" value={newAdmin.password} onChange={e => setNewAdmin({...newAdmin, password: e.target.value})} className="flex-1 border border-slate-300 rounded-xl px-4 py-2 text-sm outline-none focus:border-blue-500" />
        ...
        {/* ✅ TAB 2: MANAGEMENT NETWORKS (ปรับปรุงใหม่ 2 คอลัมน์ และย้ายปุ่ม Save) */}
        {activeTab === 'NETWORKS' && (
          <div className="animate-in fade-in flex-1 flex flex-col">
            ...
            {/* รายการ IP - จัดเรียง 2 คอลัมน์ด้วย Grid */}
            ...
            {/* แถบเพิ่ม IP และ ปุ่ม Save (ย้ายมารวมกันด้านล่างสุด) */}
